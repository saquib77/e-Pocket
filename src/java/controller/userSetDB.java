...
public class userSetDB {
    ...
    public int saveUser(User user){
        try{
            String q1 = "insert into login_master(email,password,status)values(?,?,?)";
            String q2 = "insert into country(country_code,country_name)values(?,?)";
            String q3 = "insert into state(state_id,state_name,country_code)values(?,?,?)";
            String q4 = "insert into city(city_id,city_name,state_id)values(?,?,?)";
            String q5 = "insert into user_master(uid, firstname, lastname, phone_num, city_id, email, gender, que, ans)values(?,?,?,?,?,?,?,?,?)";
            String q6 = "insert into pass_recovery(rec_id,rec_que,rec_pass,rec_time,email)values(?,?,?,?,?)";
            
            PreparedStatement pst = con.prepareStatement("select * from login_master where email=?");
            ...
            if(rs.next()) return 2;
                ...
                ps1.setString(2, user.getPass());
                ps1.setInt(3, user.getStatus());
                ps1.executeUpdate();
                
                PreparedStatement pps = con.prepareStatement("select * from country where country_name=?"); 
                pps.setString(1,user.getCountry());
                ResultSet rs1 = pps.executeQuery();
                if(rs1.next()){
                    user.setCountry_code(rs1.getInt("country_code"));
                }else{
                    PreparedStatement pps1 = this.con.prepareStatement(q2);
                    pps1.setInt(1,user.getCountry_code());
                    pps1.setString(2,user.getCountry());
                    pps1.executeUpdate();
                }
                int state_id = 1;
                Statement st = this.con.createStatement();
                ResultSet rs2 = st.executeQuery("select * from state");
                while(rs2.next()){
                    state_id = rs2.getInt("state_id");
                }
                state_id++;
                user.setState_id(state_id);
                PreparedStatement pm2 = this.con
                        .prepareStatement("select * from state where state_name=? and country_code =?");
                ...
                if (rs3.next()) {
                    ...
                    PreparedStatement pstmt3 = this.con.prepareStatement(q3);
                ...
                if (rs4.next()) {
                    ...
                    PreparedStatement pstmt4 = this.con.prepareStatement(q4);
                ...
                p++;
                PreparedStatement pstmt5 = this.con.prepareStatement(q5);
                pstmt5.setInt(1, p);
                ...
                PreparedStatement pstmt6 = this.con.prepareStatement(q6);
